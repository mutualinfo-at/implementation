...
def fgsm_whitebox(model, X, y, epsilon = 0.031):
    ...
    out_nat = model(X)
    err = (out_nat.data.max(1)[1] != y.data).float().sum()
    out_adv = model(x_adv)
...
def eval_adv_test_whitebox(model, device, test_loader):
    ...
    for data, target in test_loader:
        ...
        err_natural, err_robust = fgsm_whitebox(model, X, y)# _pgd_whitebox(model, X, y)
        #fgsm_whitebox(model, X, y)
        #_pgd_whitebox(model, X, y)
